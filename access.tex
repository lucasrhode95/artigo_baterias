...
\begin{document}
    ...
    \label{sec:introduction}
        ...
        In \iac{MG}, the role of \acp{REG} is vital and as such, \acp{ESS} are one of the main components in these environments. While traditional sources can store energy in fuels tanks, water reservoirs and kinetic energy; many renewable sources need external devices to store their output when it is abundant, to later return it to the grid \cite{STECCA2020}. In fact, with the combination of storage and \acp{REG}, an \ac{EMS} can coordinate resources to achieve increased system efficiency by flattening the demand curve compensating for generation-demand mismatches, using techniques such as time-shifting, peak shaving and valley filling \cite{WANG20196201, LI2020106058, PARRA2015576, ZHANG2019772}.

        There are many different types of devices that store energy, however,\ac{BESS} are usually the most technical fit choice for energy management applications. It presents good response time and cycle life, good efficiency and technical maturity that made it emerge as one of the main forms of energy storage in \acp{MG} \cite{KOCER2019, martins2018optimal, FU20136749070}.

        Lithium-ion and lead-acid are the main of batteries chemistry used in modern \ac{BESS} applications \cite{FU20136749070, ALSAIDAN8094981}. Historically, lead-acid batteries were preferred for their lower upfront, but recently electric vehicles and associated mass production have drastically reduced the cost of lithium-ion cells \cite{BBERG2020} with some sources indicating an equality between the cost per kWh of the two technologies in 2020\cite{mongird20202020}. This makes this type of batteries a more attractive choice for its greater overall performance and durability \cite{wang2013li, xu2010lithium}.

        Yet, prolonging the durability of battery cells is still an ongoing study, with manufacturers and researches exploring new compositions that are expected to deliver more durable cells \cite{mainar2018systematic, xie2018long, wang2019high, shen2019li}. Furthermore, even with the aforementioned price reduction, the capital investment and replacement cost of batteries is currently high \cite{mongird20202020, liao2018bess}, which is aggravated by the limited lifespan of these devices when compared to other power grid components \cite{mun2021study}. For that reason, it is important to optimize the operation of these devices to maximize the benefit they bring to the grid.
